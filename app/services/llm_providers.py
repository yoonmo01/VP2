...
def _openai_like_chat(model: str,
                      ...
                      api_key: str,
                      temperature: float = 0.7):
    """로컬 OpenAI 호환 서버용 (옵션). 현재 기본 경로는 Gemini."""
    return ChatOpenAI(
        model=model,
        base_url=base_url,
        api_key=api_key,
...
def openai_chat(model: Optional[str] = None, temperature: float = 0.7):
    """백업용: 혹시나 필요 시 남겨둠. 기본 경로는 쓰지 않음."""
    if not settings.OPENAI_API_KEY:
        raise RuntimeError("OPENAI_API_KEY not set")
    mdl = model or settings.ADMIN_MODEL
    is_o_series = mdl.strip().lower().startswith("o")
    if is_o_series:
        return ChatOpenAI(
            model=mdl,
            temperature=1,
...
def agent_chat(model: Optional[str] = None, temperature: float = 0.2):
    """
    에이전트(리즌/계획/조정): 추론형 모델 권장 → gemini-1.5-pro
    """
    use_model = model or getattr(settings, "AGENT_MODEL", "gemini-1.5-pro")
    # 에이전트는 항상 추론형으로 고정
    use_model = "gemini-1.5-pro"
    return gemini_chat(use_model, temperature=temperature)


def attacker_chat():
    """
    피싱범(공격자): 속도/가성비 → gemini-2.5-flash-lite
    """
    use_model = getattr(settings, "ATTACKER_MODEL", "gemini-2.5-flash-lite")
    use_model = "gemini-2.5-flash-lite"
    return gemini_chat(use_model, temperature=0.7)


def victim_chat():
    """
    피해자: 속도/가성비 → gemini-2.5-flash-lite
    """
    use_model = getattr(settings, "VICTIM_MODEL", "gemini-2.5-flash-lite")
    use_model = "gemini-2.5-flash-lite"
    return gemini_chat(use_model, temperature=0.7)
...
def admin_chat():
    """
    관리자(판정/요약/강화지침): 정확성/일관성 → gemini-1.5-pro
    """
    use_model = getattr(settings, "ADMIN_MODEL", "gemini-1.5-pro")
    use_model = "gemini-1.5-pro"
    return gemini_chat(use_model, temperature=0.7)

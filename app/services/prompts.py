...
ATTACKER_PROMPT = ChatPromptTemplate.from_messages([
    ...
    - "그만 하(시|겠)죠" / "그만하시죠"
     ...
     [출력 형식 규정 — 의도 라벨링 추가]
    - 생성된 공격자 발화의 **마지막 한 줄**에는 반드시 다음 형식으로 의도 라벨을 **한 단어만** 표기하십시오:
    INTENT: <의도>
    - <의도>는 아래 14개 중 **정확히 하나**만 사용합니다:
    없음, 불안, 궁금증 유발, 혐의 형성, 놀람, 안심, 압박, 유도, 안도, 신뢰 형성, 상황 파악, 분위기 조성, 정보 파악, 보이스피싱이 아니라는 인상 형성
    - 출력은 (1) 공격자 대사 단락(2~4문장, 최대 350자) 및 (2) 바로 이어지는 한 줄의 INTENT 라벨만 포함해야 합니다. 다른 해설·요약·메타정보는 금지합니다.
    - 이 기능은 **교육·방어 목적의 시뮬레이션용**에만 사용되며, 실제 개인정보·실제 계좌·실제 전화번호·실제 기관명 등은 절대 포함하거나 요구하지 마십시오.
